...
FOCUS_FEATURE = "nirK_aa_frac_W" if MODEL == "A" else "nirK_aa_frac_W"

# Protein length column, used to convert the fraction back into a residue count..
...
# Colours are distinguishable in greyscale and under common forms of colour
# vision deficiency.
...
def _lineage_rank(cur, taxid, rank, cache):
    """Walk up the NCBI taxonomy tree from taxid to the ancestor at `rank`."""
    key = (taxid, rank)
    if key in cache:
        return cache[key]
    seen = set()
